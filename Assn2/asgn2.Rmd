...
barplot(importance$Value, main = "Variable importance (Breiman method)", xlab = "variables", ylab = "Importance", names.arg = rownames(importance), cex.names = 0.95, ylim = c(0, max(importance$Value) + 2))
```
...
for (var in 1:100){
  ...
  train <- data[[1]]
  test <- data[[2]]
  dt.mod <- build.tree(train, Class ~., maxdepth=30)
  predictions <- predict.tree(dt.mod, test)
  results <- table(test$Class, predictions)
  dt[var] <- sum(diag(results))/sum(results)
  ...
  log[var] <- (conf.mat$table[1]+conf.mat$table[4])/nrow(test)
}
par(mfrow = c(1,2))
boxplot(dt, main = "Dt accuracy", ylim = c(0.5,1.0))
boxplot(log, main = "logistic accuracy", ylim = c(0.5,1.0))
...
boxplot(results, main = "Model accuracy scores", xlab = "models", ylab = "Accuracy")
```
